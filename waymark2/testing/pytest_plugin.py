"""Pytest plugin: ``pytest --waymark2`` runs the 2.0 conformance suite
...
- a ``waymark2_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm2``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark2") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark2.testing.conformance", "waymark2.testing.walker"):
        return
    kinds = _selected_kinds(metafunc.config)
    principals = sorted(reg.principals())

    if "wm2_case" in metafunc.fixturenames:
        cases, ids = [], []
        for kind in kinds:
        ...
        metafunc.parametrize("wm2_case", cases, ids=ids)

    if "wm2_action_case" in metafunc.fixturenames:
        cases, ids = [], []
        for kind in kinds:
            cls = reg.resources()[kind]
        ...
        metafunc.parametrize("wm2_action_case", cases, ids=ids)

    if "wm2_kind" in metafunc.fixturenames:
        metafunc.parametrize("wm2_kind", kinds, ids=kinds)


@pytest.fixture
async def wm2(waymark2_engine):
    """The conformance environment: engine + client with principal injection."""
    from fastapi import FastAPI
    from httpx import ASGITransport, AsyncClient

    engine = waymark2_engine
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark2-conformance")
    try:
