"""--waymark2-walk: random-walk each resource's *real* transitions from its
    ...
    for walk in range(WALKS):
        ...
        for step in range(MAX_STEPS):
            ...
            async with wm2.storage.session() as s:
                current = await wm2.storage.load(s, kind, instance.id)
            body = await build_input(wm2, kind, defn, current)
            headers = H(pname)
            if defn.safety.requires_if_match:
                headers["If-Match"] = doc["meta"]["etag"]
            if not defn.safety.idempotent:
                headers["Idempotency-Key"] = uuid.uuid4().hex
            res = await wm2.client.post(f"{doc['self']}/-/{action}",
