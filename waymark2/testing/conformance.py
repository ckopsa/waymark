...
Collected when pytest runs with ``--waymark2``. Parametrized per resource ×
...
async def make(wm2: Any, kind: str, state: str) -> Any:
    try:
        instance = await reg.make_state(kind, state, wm2.engine)
...
async def fetch(wm2: Any, kind: str, id: str, pname: str) -> dict[str, Any]:
    rdef = wm2.registry[kind]
    res = await wm2.client.get(f"{wm2.base}/{rdef.plural}/{id}", headers=H(pname))
    assert res.status_code == 200, res.text
    return res.json()


async def build_input(wm2: Any, kind: str, defn: ActionDef,
    ...
    try:
        return await reg.synthesize_input(wm2.engine, wm2.registry[kind],
                                          defn, instance)
    except reg.SkipState as e:
        pytest.skip(str(e))
...
async def post(wm2: Any, doc: dict[str, Any], defn: ActionDef, pname: str,
    ...
    return await wm2.client.post(href, json=body, headers=headers)


async def storage_version(wm2: Any, kind: str, id: str) -> int:
    async with wm2.storage.session() as s:
        loaded = await wm2.storage.load(s, kind, id)
    return loaded.version


async def transition_count(wm2: Any) -> int:
    from sqlalchemy import func, select

    async with wm2.storage.session() as s:
        return (await s.execute(
            select(func.count()).select_from(wm2.storage.transitions))).scalar_one()


async def principals_with(wm2: Any, kind: str, instance_state: str,
    ...
    out = []
    for pname in sorted(wm2.principals):
        instance = await make(wm2, kind, instance_state)
        doc = await fetch(wm2, kind, instance.id, pname)
...
# ── Representation ──────────────────────────────────────────────────────
async def test_representation(wm2, wm2_case):
    kind, state, pname = wm2_case
    instance = await make(wm2, kind, state)
    doc = await fetch(wm2, kind, instance.id, pname)

    missing = ENVELOPE_KEYS - set(doc)
    assert not missing, f"envelope missing required keys: {missing}"
    assert doc["waymark"] == "2"
    ...
    schema_res = await wm2.client.get(f"{wm2.base}/schemas/{kind}")
    assert schema_res.status_code == 200
    jsonschema.validate(doc["data"], schema_res.json())

    assert doc["meta"]["version"] == await storage_version(wm2, kind, instance.id)
...
async def test_affordance_completeness(wm2, wm2_case):
    kind, state, pname = wm2_case
    rdef: ResourceDef = wm2.registry[kind]
    instance = await make(wm2, kind, state)
    doc = await fetch(wm2, kind, instance.id, pname)
    ...
    for name in hidden:
        ...
        body = await build_input(wm2, kind, defn, instance)
        res = await post(wm2, doc, defn, pname, body)
...
async def test_token_prose(wm2, wm2_case):
...
async def test_orientation(wm2, wm2_kind):
    ...
    for state in rdef.machine.states:
        try:
            ...
            continue
        doc = await fetch(wm2, kind, instance.id, sorted(wm2.principals)[0])
...
# ── Transition truth ────────────────────────────────────────────────────
async def test_transition_truth_available(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    rdef = wm2.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm2, kind, state, action, "actions")
    if not cases:
    ...
    for pname, instance, doc in cases:
        ...
        async with wm2.storage.session() as s:
            last = await wm2.storage.last_transition(s, kind, instance.id)
        assert last.action == action
        assert last.from_state == state and last.to_state == defn.to
        assert last.actor_id == wm2.principals[pname].id
        assert last.actor_type == wm2.principals[pname].type

        refetched = await fetch(wm2, kind, instance.id, pname)
        for field in ("state", "data", "meta", "actions"):
            assert refetched[field] == after[field], \
                "response is not the post-transition document"


async def test_transition_truth_unavailable(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    rdef = wm2.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm2, kind, state, action, "unavailable")
    if not cases:
...
# ── Safety truth ────────────────────────────────────────────────────────
async def test_safety_idempotent_double_invoke(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    defn = wm2.registry[kind].machine.actions[action]
    if not defn.safety.idempotent:
    ...
    body = await build_input(wm2, kind, defn, instance)
    first = await post(wm2, doc, defn, pname, body)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200
    v1 = first.json()["meta"]["version"]

    second = await post(wm2, doc, defn, pname, body,
...
async def test_safety_non_idempotent_key_discipline(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    defn = wm2.registry[kind].machine.actions[action]
    ...
    key = uuid.uuid4().hex
    first = await post(wm2, doc, defn, pname, body, idem_key=key)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200, first.text

    replay = await post(wm2, doc, defn, pname, body, idem_key=key)
    ...
    if defn.input is not None:
        ...
        reuse = await post(wm2, doc, defn, pname, mutated, idem_key=key)
        assert reuse.status_code == 409, \
            f"key reuse with different body must 409, got {reuse.status_code}"


async def test_safety_if_match(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    defn = wm2.registry[kind].machine.actions[action]
    ...
    pname, instance, doc = cases[0]
    body = await build_input(wm2, kind, defn, instance)
    stale = await post(wm2, doc, defn, pname, body, etag='W/"conformance-stale"')
...
async def test_safety_reversible(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    defn = wm2.registry[kind].machine.actions[action]
...
# ── Input contract ──────────────────────────────────────────────────────
async def test_input_contract(wm2, wm2_action_case):
    kind, state, action = wm2_action_case
    rdef = wm2.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm2, kind, state, action, "actions")
    ...
    if defn.input is not None:
        junk = await post(wm2, doc, defn, pname, {"__conformance_bogus__": 1})
    ...
    body = await build_input(wm2, kind, defn, instance)
    v_before = await storage_version(wm2, kind, instance.id)
    t_before = await transition_count(wm2)
    dry = await post(wm2, doc, defn, pname, body, dry_run=True)
    ...
    assert await storage_version(wm2, kind, instance.id) == v_before, \
        "dry_run changed the resource version"
    assert await transition_count(wm2) == t_before, \
...
async def test_schema_guard_gap(wm2, wm2_action_case):
    ...
    async with wm2.storage.session() as s:
        ctx = wm2.engine.invoker._ctx(wm2.principals[pname], s, mode="dry_run")
        for g in derivable:
            denies = 0
            for inp in parsed:
...
async def test_prefill_truth(wm2, wm2_action_case):
...
async def test_draft_protection(wm2, wm2_action_case):
    ...
    href = advert["href"]
    empty = await wm2.client.get(href, headers=H(pname))
    ...
    body = await build_input(wm2, kind, defn, instance)
    res = await wm2.client.put(href, json=body, headers=H(pname))
    assert res.status_code == 200, res.text
    saved = res.json()
    ...
    fresh = await wm2.client.get(href, headers=H(pname))
    ...
    for other in sorted(wm2.principals):
        if other == pname:
            continue
        other_get = await wm2.client.get(href, headers=H(other))
    ...
    junk = await wm2.client.put(href, json={"__not_a_field__": 1},
                                headers=H(pname))
    assert junk.status_code == 422, \
        "draft fields outside the action's schema must be rejected"

    res = await post(wm2, refetched, defn, pname, body)
    fail_if_needs_example(res, kind, defn)
    assert res.status_code == 200, res.text
    after_get = await wm2.client.get(href, headers=H(pname))
...
# ── Collection contract ─────────────────────────────────────────────────
async def test_collection_contract(wm2, wm2_kind):
    kind = wm2_kind
    rdef = wm2.registry[kind]
    cls = rdef.cls
    made = []
    for state in cls.__waymark_machine__.states:
        try:
            made.append(await reg.make_state(kind, state, wm2.engine))
        except reg.SkipState:
            continue
    if not made:
        pytest.skip("no states producible")
    pname = sorted(wm2.principals)[0]
    base = f"{wm2.base}/{rdef.plural}"

    async def query(qs: str) -> dict:
        res = await wm2.client.get(f"{base}?{qs}", headers=H(pname))
        assert res.status_code == 200, res.text
        return res.json()
    ...
    while True:
        ...
        if not nxt:
            break
        res = await wm2.client.get(nxt["href"], headers=H(pname))
        assert res.status_code == 200, res.text
        doc = res.json()
...
async def test_bulk_report(wm2, wm2_kind):
    ...
    for defn in bulk_actions:
        ...
        body = dict(await build_input(wm2, kind, defn) or {})
        body["ids"] = ids
        headers = H(sorted(wm2.principals)[0])
        if not defn.safety.idempotent:
            headers["Idempotency-Key"] = uuid.uuid4().hex
        res = await wm2.client.post(f"{wm2.base}/{rdef.plural}/-/{defn.name}",
...
# ── Events ──────────────────────────────────────────────────────────────
async def test_events(wm2, wm2_kind):
    ...
    rdef = wm2.registry[kind]
    machine = rdef.cls.__waymark_machine__
    instance = await make(wm2, kind, machine.initial)
    ...
    sub = wm2.engine.dispatcher.subscribe(resource=(kind, instance.id))
    try:
        body = await build_input(wm2, kind, defn, instance)
        res = await post(wm2, chosen_doc, defn, principal, body)
        fail_if_needs_example(res, kind, defn)
        assert res.status_code == 200, res.text
        after = res.json()
...
async def test_migration_roundtrip(wm2):
    ...
    snapshot = wm2.storage.schema_snapshot()
    stmts = m.diff_statements(wm2.storage, None)

    scratch = "waymark2_conformance_migration"
    async with wm2.storage.engine.begin() as conn:
...
# ── Suite integrity ─────────────────────────────────────────────────────
async def test_every_resource_is_enrolled(wm2):
    missing = [k for k in wm2.registry.kinds() if k not in reg.resources()]
    assert not missing, (
