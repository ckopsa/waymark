...
- ``waymark2 migrate`` snapshots the registry's storage schema
...
- Revisions are plain ``NNNN_*.sql`` files under ``migrations/waymark2/``,
  applied in order and recorded in ``waymark2_schema_migrations`` —
...
MIGRATIONS_TABLE = "waymark2_schema_migrations"
...
def emit(storage: PostgresStorage, directory: Path, *,
    ...
    header = ("-- waymark2 migration: generated from the declared registry.\n"
...
async def apply(dsn: str, directory: Path) -> list[str]:
    ...
    ``waymark2_schema_migrations``. Refuses files with unresolved REVIEW
