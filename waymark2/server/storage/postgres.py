...
- Engine tables are ``waymark2_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark2 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark2_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark2_transitions", self.metadata,
            ...
            Index("ix_waymark2_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark2_idempotency", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark2 migrate``'s
