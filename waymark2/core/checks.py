...
def check_guard_templates(cls: type, machine: StateMachine) -> None:
    """Static template check where vars are declarable; guards with a
    ``vars`` callable are covered empirically by conformance token_prose."""
    for name, defn in machine.actions.items():
        for g in _leaf_guards(defn):
            if g.declared_vars is None:  # vars=fn(r): checked at conformance
                continue
            placeholders = {
    ...
    for name, defn in machine.actions.items():
        for g in _leaf_guards(defn):
            ...
            if g.accepts is not None:
                covered.add(g.judges[0])
            if g.relates is not None:
                covered.update((g.relates[0], g.relates[2]))
            for f in g.judges:
                if f in covered:
                    continue
