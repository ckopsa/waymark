"""The waymark2 CLI: check | routes | openapi | migrate | new-resource
...
app = typer.Typer(help="Waymark 2.0: affordance-oriented hypermedia framework tools.",
    ...
    for rdef in engine.registry.defs():
        ...
        for name, defn in m.actions.items():
            ...
            flags = [f for f, v in (("idempotent", defn.safety.idempotent),
                                    ...
                                    ("if-match", defn.safety.requires_if_match),
                                    ("bulk", defn.bulk)) if v]
...
def migrate(engine_spec: str = typer.Argument(
        "app.main:engine", help="module:attr of the Engine"),
        directory: Path = typer.Option(Path("migrations/waymark2"),
...
from waymark2 import Acknowledged, Ctx, Resource, Safety, action
