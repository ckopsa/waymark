# The family meal planner (waymark7)

The meal planner on Waymark 7.0 — the mealplan6 app forked onto the
"law binds rows" runtime (`docs/waymark7-design.md`). The calendar
...
```bash
make mealplan7                               # db + wheel + uvicorn on :8005
...
This app runs on **waymark7** (see `docs/waymark7-design.md`). Meal,
...
```bash
uv run waymark7 check mealplan7.main:engine
make conformance7                     # pytest --waymark7 -n auto
uv run pytest --waymark7 plan         # or meal, event, rotation, …
```
