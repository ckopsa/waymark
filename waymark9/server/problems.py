...
class FactRecomputing(Problem):
    """503: the filter names a derived fact whose definition changed this
    deploy and whose stored values are still catching up under a declared
    ``Deferred`` backfill (design §4). The Service-down honesty, applied
    to truth: the fact is un-advertised, not wrong — ``recomputing``
    lists what is catching up, and the parameter returns when the
    backfill completes."""

    slug = "fact-recomputing"
    title = "Fact is recomputing"
    status = 503
