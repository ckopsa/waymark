...
log = logging.getLogger("waymark9.jobs")
...
class Invoker:
    def __init__(self, *, registry: Registry, storage: Any,
        ...
        # the definition-lifecycle seam (design 7.0 §1–§2): set by the
        # engine at startup; rides every ctx (the definition kind's own
        # handlers consult it) and gets the entry-point after-commit call
        # a human ``promote`` needs (law flip + backfill run post-commit)
    ...
    async def _append(self, s: Any, *, kind: str, instance: Any, action: str,
        ...
        ``inputs`` is the declared-retention payload (design 7.0 §5) —
        None keeps the digest-only default."""
    ...
    async def invoke(self, kind: str, id: str, action_name: str,
        ...
        if self.lifecycle is not None:
            # the definition lifecycle's post-commit half (design 7.0 §1):
            # a committed ``promote`` flips the served law, drops the §1
            # overlay, and runs the stale-by-definition backfill — work
            # that must not run inside the act's own transaction. A no-op
            # for every other kind/action; idempotent for replays.
            await self.lifecycle.after_commit(kind, id, action_name, result)
        return result
    ...
    async def _create_core(self, s: Any, ctx: Ctx, rdef: ResourceDef,
        ...
        # the declared create landing (design 7.0 §1, Resource.created_in):
        # a kind whose creations may be born beyond the machine's initial
        # (the definition kind's propose-mode `proposed`) declares every
        # such state; an undeclared landing is refused, never improvised
        landing = instance.created_in()
        if landing != rdef.machine.initial:
            from ..core.types import DefinitionError

            if landing not in rdef.cls.create_state_names \
                    or landing not in rdef.machine.states:
                raise DefinitionError(
                    f"{rdef.kind}.created_in() returned {landing!r}, which "
                    "create_state_names does not declare — where a creation "
                    "lands is declared, never improvised")
        ...
        created = await self._append(
            ...
            # unconditional retention (design 7.0 §5): a record_inputs
            # kind's create stores its validated payload — the definition
            # kind's revise/propose rows carry the law they wrote
            inputs=(data.model_dump(mode="json")
                    if rdef.cls.record_inputs else None),
        )
    ...
    async def _invoke_in_session(
        ...
        summary = render_summary(rdef.summary_template, instance)
        # declared retention (design 7.0 §5): the validated input payload
        # lands on the transition row where the action said record=Inputs()
        # — or unconditionally for a record_inputs kind (the definition's
        # own transitions; the law does not get privacy from its subjects)
        recorded = None
        if inp is not None and (defn.record is not None
                                or rdef.cls.record_inputs):
            recorded = inp.model_dump(mode="json")
        transition = await self._append(
    ...
    async def _run_service_job(self, job_id: str, service: Any,
        ...
        try:
            ...
            for index, (_name, args) in enumerate(artifacts):
                ...
                if job is None or job.state == "cancelled":
                    return
                outcome, message, detail = "succeeded", None, None
                if down is not None:
                    outcome, message = "failed", down
                else:
                    try:
                        result = await service.call(*args, now=self.clock())
                        if isinstance(result, dict):
                            # a structured result (design 7.0 §2: the
                            # blast-radius report) lands on the artifact
                            detail = result
