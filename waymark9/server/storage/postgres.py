...
- Engine tables are ``waymark9_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark9 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark9_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark9_transitions", self.metadata,
            ...
            # the waymark9_schema_migrations mechanism: the column is
            # nullable, so the emitted ALTER is a plain ADD COLUMN.
            Column("inputs", JSONB, nullable=True),
            Index("ix_waymark9_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark9_idempotency", self.metadata,
        ...
        self.webhook_cursors = Table(
            "waymark9_webhook_cursors", self.metadata,
        ...
        self.cursors = Table(
            "waymark9_cursors", self.metadata,
        ...
        self.job_leases = Table(
            "waymark9_job_leases", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark9 migrate``'s
