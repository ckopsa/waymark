...
class Inputs:
    """Declared retention for the transition log (design 7.0 §5).

    ``@action(..., record=Inputs())``: the transition row stores the
    validated input payload itself, not only its digest — reviewing a
    proposal, auditing a withdrawal, forensics a year later all read the
    actor's words next to the ``defined_by`` stamp. Opt-in per action:
    everything undeclared keeps the digest, and historical as-of ships
    exactly as far as retention was declared. An action with no input
    model has nothing to record and refuses the declaration at import.
    """
...
class ActionDef:
    ...
    # declared input retention (design 7.0 §5): an Inputs() declaration
    # makes the transition row store the validated payload, not only its
    # digest — the invoker's append is the one consumer
    record: "Inputs | None" = None
...
def action(
    *,
    ...
    if record is not None:
        if not isinstance(record, Inputs):
            raise DefinitionError(
                f"@action record= takes an Inputs() declaration, got "
                f"{record!r}")
        if input is None:
            raise DefinitionError(
                "@action record=Inputs() requires input= — an action with "
                "no input model has nothing to record (design 7.0 §5)")
