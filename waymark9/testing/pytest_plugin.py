"""Pytest plugin: ``pytest --waymark9`` runs the 2.0 conformance suite
...
- a ``waymark9_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm9``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark9") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark9.testing.conformance", "waymark9.testing.walker"):
        return
...
async def wm9(waymark9_engine):
    ...
    # (waymark9.server.external.refresh_mirror) suppresses pull-through for
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark9-conformance")
    try:
