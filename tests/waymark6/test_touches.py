...
@pytest.fixture
async def env():
    engine = waymark6.Engine(resources=[XEvent, XRow], storage=TEST_DSN,
...
async def test_undeclared_writes_refuse_naming_the_declaration(env):
    ...
    with pytest.raises(DefinitionError) as err:
        await engine.invoker.invoke(
            ...
            principal=waymark6.Principal(id="fern", type="human"),
...
async def test_defer_and_seeds_stay_lawful(env):
    ...
    from waymark6.server.grants import ApprovalRequest
