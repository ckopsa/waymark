...
import waymark6
from waymark6 import Authored, Ctx, Resource, Safety, action
from waymark6.core.guards import Guard
from waymark6.core.types import Acknowledged, Allow, DefinitionError, Deny
from waymark6.server.bus import InProcessBus
from waymark6.server.engine import header_principal
from waymark6.server.external import AuthoredMeta, Discover
...
class Entity(Resource):
    ...
    filterable = waymark6.filterable(
        state=waymark6.filterable.Eq | waymark6.filterable.In,
        external_id=waymark6.filterable.Eq,
    )

    @action(from_=EntityState.ONBOARDING, to=EntityState.ONBOARDING,
            input=CrosswalkInput,
            edit=waymark6.Edit(prefill=("crosswalk_id",), fence=False,
...
async def env():
    ...
    CRM.pulls = 0
    CRM.discoverable = []
    engine = waymark6.Engine(resources=[Entity], storage=TEST_DSN,
    ...
    try:
        with pytest.raises(DefinitionError, match="authored"):
            await engine.invoker.invoke(
                ...
                principal=waymark6.Principal(id="dana"),
