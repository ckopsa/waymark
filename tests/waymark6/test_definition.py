...
import waymark6
from waymark6 import Allow, Ctx, Deny, Derived, Guard, Resource, Safety, action
from waymark6.core.fingerprint import fingerprint_hash, fingerprint_of
from waymark6.server.bus import InProcessBus
from waymark6.server.definitions import REGISTRY_KIND
...
async def _boot(doc_cls, *, drop: bool = False):
    engine = waymark6.Engine(resources=[doc_cls], storage=TEST_DSN,
...
async def test_changed_guard_explain_is_a_judgment_revision():
    ...
    try:
        ...
        assert superseded.actor_id == "waymark6-deploy"
...
def _doc_hash() -> str:
    ...
    from waymark6.core.registry import Registry
    from waymark6.server.storage.postgres import PostgresStorage
...
async def test_fingerprint_determinism_across_processes():
    ...
    out = subprocess.run(
        [sys.executable, "-c",
         "from tests.waymark6.test_definition import _doc_hash; "
         "print(_doc_hash())"],
