...
import waymark8
from waymark8 import Ctx, DraftPolicy, Edit, Resource, Safety, action
from waymark8.client import AgentClient, mcp_tools
...
async def env():
    ...
    engine = waymark8.Engine(resources=[Note], storage=TEST_DSN,
...
async def test_state_graph_skips_non_envelope_docs():
    ...
    learner. Regression: `waymark8 client get …/-/grantable` raised
    KeyError('kind') because learn() assumed a resource envelope."""
    from waymark8.client.agent import StateGraph
    from waymark8.client.py import Doc
