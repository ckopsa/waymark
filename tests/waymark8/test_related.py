...
NOTE (waymark8-notes): the design doc spells the citation
...
from waymark8.core import checks
...
def test_related_field_must_name_a_real_target_field():
    class D(BaseModel):
        start_date: date
        end_date: date
        n: bool = waymark8.Derived(over=(_calendar.field("nope"),),
...
@pytest.fixture
async def env():
    engine = waymark8.Engine(resources=[RPlan, REvent], storage=TEST_DSN,
...
async def test_the_predicate_rides_the_fingerprint(env):
    ...
    from waymark8.core.fingerprint import fingerprint_of
