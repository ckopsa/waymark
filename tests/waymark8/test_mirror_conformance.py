...
a pure :class:`~waymark8.server.external.Mirror` — a resource that breaks
...
- the Mirror-aware state factory stages every sync state through the system
  actor (``waymark8.testing.factories.make_mirror_state``), and
...
from waymark8.server.external import Discover, Mirror, MirrorMeta, SyncState
from waymark8.testing import (
    factories as reg,
    per_worker_dsn,
)
from waymark8.testing import conformance as conf
from waymark8 import filterable
...
# writes to the process-global conformance registry, and under ``--waymark8``
...
@pytest.fixture
async def wm8():
    """A conformance environment for the mirror, mirroring the --waymark8
    plugin's ``wm8`` fixture — including the walker-scoped suppression seam."""
    SOURCE.docs.clear()
    SOURCE.etags.clear()
    ...
    engine = waymark8.Engine(resources=[NoteMirror], storage=TEST_DSN,
    ...
    client = AsyncClient(transport=transport,
                         base_url="http://waymark8-conformance")
    try:
...
async def test_representation_holds_the_staged_states(wm8):
    ...
    for state in SYNC_STATES:
        for pname in PRINCIPALS:
            try:
                await conf.test_representation(wm8, (KIND, state, pname))
                ran += 1
            except _SKIPPED:
                continue
...
async def test_affordance_and_prose(wm8):
    ...
    for state in SYNC_STATES:
        await conf.test_affordance_completeness(wm8, (KIND, state, "owner"))
        await conf.test_token_prose(wm8, (KIND, state, "owner"))


async def test_orientation(wm8):
    """Summaries differ across the staged sync states — a Mirror that could
    only ever be walked to ``fresh`` could never prove this."""
    await conf.test_orientation(wm8, KIND)


async def test_collection_contract(wm8):
    await conf.test_collection_contract(wm8, KIND)


async def test_replay_history(wm8):
    ...
    await conf.test_replay_history(wm8, KIND)


async def test_events(wm8):
    ...
    failure."""
    try:
        await conf.test_events(wm8, KIND)
    except _SKIPPED:
...
async def test_sync_transitions_are_hidden_not_advertised(wm8):
    ...
    for state in SYNC_STATES:
        ...
        for pname in PRINCIPALS:
            doc = await conf.fetch(wm8, KIND, instance.id, pname)
...
async def test_reconcile_is_a_real_principal_transition(wm8):
    ...
    await conf.test_transition_truth_available(
        wm8, (KIND, "conflicted", "reconcile"))
    await conf.test_input_contract(wm8, (KIND, "conflicted", "reconcile"))
    ...
    await conf.test_transition_truth_unavailable(
        wm8, (KIND, "fresh", "reconcile"))


# ── staging honesty: each sync state is produced as asked ───────────────
async def test_factory_stages_each_sync_state(wm8):
    for state in SYNC_STATES:
        instance = await reg.make_state(KIND, state, wm8.engine)
...
async def test_discover_minted_instance_reads_cleanly(wm8):
    ...
    from waymark8.server.external import run_discovery

    SOURCE.discoverable = ["deal-9001"]
    future = wm8.engine.invoker.clock() + timedelta(days=2)
    minted = await run_discovery(wm8.engine, future)
    assert minted == 1

    async with wm8.storage.session() as s:
        rows, _ = await wm8.storage.query(
            s, KIND, filters={"external_id": "deal-9001"}, sort=None,
            page_size=1, page_number=1)
    assert rows, "discovery minted no resource"
    doc = await conf.fetch(wm8, KIND, rows[0].id, PRINCIPALS[0])
