...
import waymark4
from waymark4 import Authored, Ctx, Resource, Safety, action
...
from waymark4.server.external import AuthoredMeta, Discover
...
class Entity(Resource):
    ...
    filterable = waymark4.filterable(
        state=waymark4.filterable.Eq | waymark4.filterable.In,
        external_id=waymark4.filterable.Eq,
    )

    @action(from_=EntityState.ONBOARDING, to=EntityState.ONBOARDING,
            input=CrosswalkInput,
            edit=waymark4.Edit(prefill=("crosswalk_id",), fence=False,
...
async def env():
    ...
    CRM.pulls = 0
    CRM.discoverable = []
    engine = waymark4.Engine(resources=[Entity], storage=TEST_DSN,
    ...
    try:
        with pytest.raises(DefinitionError, match="authored"):
            await engine.invoker.invoke(
                ...
                principal=waymark4.Principal(id="dana"),
