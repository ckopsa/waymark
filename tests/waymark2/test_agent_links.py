...
import waymark2
from waymark2 import Ctx, Edit, Resource, Safety, action
...
async def env():
    ...
    engine = waymark2.Engine(resources=[Memo], storage=TEST_DSN,
...
async def _mint(human) -> tuple[str, str]:
    ...
    res = await human.post("/api/agent_grants",
                           json={"agent_name": "Robo"},
...
async def _act(client, doc, action, body=None, extra=None):
    ...
    if entry and entry.get("safety", {}).get("requires_if_match"):
