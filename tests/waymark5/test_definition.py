...
import waymark5
from waymark5 import Allow, Ctx, Deny, Derived, Guard, Resource, Safety, action
from waymark5.core.fingerprint import fingerprint_hash, fingerprint_of
from waymark5.server.bus import InProcessBus
from waymark5.server.definitions import REGISTRY_KIND
...
async def _boot(doc_cls, *, drop: bool = False):
    engine = waymark5.Engine(resources=[doc_cls], storage=TEST_DSN,
...
def _doc_hash() -> str:
    ...
    from waymark5.core.registry import Registry
    from waymark5.server.storage.postgres import PostgresStorage
...
async def test_fingerprint_determinism_across_processes():
    ...
    out = subprocess.run(
        [sys.executable, "-c",
         "from tests.waymark5.test_definition import _doc_hash; "
         "print(_doc_hash())"],
