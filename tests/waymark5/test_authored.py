...
import waymark5
from waymark5 import Authored, Ctx, Resource, Safety, action
from waymark5.core.guards import Guard
from waymark5.core.types import Acknowledged, Allow, DefinitionError, Deny
from waymark5.server.bus import InProcessBus
from waymark5.server.engine import header_principal
from waymark5.server.external import AuthoredMeta, Discover
...
class Entity(Resource):
    ...
    filterable = waymark5.filterable(
        state=waymark5.filterable.Eq | waymark5.filterable.In,
        external_id=waymark5.filterable.Eq,
    )

    @action(from_=EntityState.ONBOARDING, to=EntityState.ONBOARDING,
            input=CrosswalkInput,
            edit=waymark5.Edit(prefill=("crosswalk_id",), fence=False,
...
async def env():
    ...
    CRM.pulls = 0
    CRM.discoverable = []
    engine = waymark5.Engine(resources=[Entity], storage=TEST_DSN,
    ...
    try:
        with pytest.raises(DefinitionError, match="authored"):
            await engine.invoker.invoke(
                ...
                principal=waymark5.Principal(id="dana"),
