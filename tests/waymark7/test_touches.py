...
@pytest.fixture
async def env():
    engine = waymark7.Engine(resources=[XEvent, XRow], storage=TEST_DSN,
...
async def test_undeclared_writes_refuse_naming_the_declaration(env):
    ...
    with pytest.raises(DefinitionError) as err:
        await engine.invoker.invoke(
            ...
            principal=waymark7.Principal(id="fern", type="human"),
...
async def test_defer_and_seeds_stay_lawful(env):
    ...
    from waymark7.server.grants import ApprovalRequest
