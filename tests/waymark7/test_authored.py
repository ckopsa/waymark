...
import waymark7
from waymark7 import Authored, Ctx, Resource, Safety, action
from waymark7.core.guards import Guard
from waymark7.core.types import Acknowledged, Allow, DefinitionError, Deny
from waymark7.server.bus import InProcessBus
from waymark7.server.engine import header_principal
from waymark7.server.external import AuthoredMeta, Discover
...
class Entity(Resource):
    ...
    filterable = waymark7.filterable(
        state=waymark7.filterable.Eq | waymark7.filterable.In,
        external_id=waymark7.filterable.Eq,
    )

    @action(from_=EntityState.ONBOARDING, to=EntityState.ONBOARDING,
            input=CrosswalkInput,
            edit=waymark7.Edit(prefill=("crosswalk_id",), fence=False,
...
async def env():
    ...
    CRM.pulls = 0
    CRM.discoverable = []
    engine = waymark7.Engine(resources=[Entity], storage=TEST_DSN,
    ...
    try:
        with pytest.raises(DefinitionError, match="authored"):
            await engine.invoker.invoke(
                ...
                principal=waymark7.Principal(id="dana"),
