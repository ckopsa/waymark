...
import waymark9
from waymark9 import Authored, Ctx, Resource, Safety, action
from waymark9.core.guards import Guard
from waymark9.core.types import Acknowledged, Allow, DefinitionError, Deny
from waymark9.server.bus import InProcessBus
from waymark9.server.engine import header_principal
from waymark9.server.external import AuthoredMeta, Discover
...
class Entity(Resource):
    ...
    filterable = waymark9.filterable(
        state=waymark9.filterable.Eq | waymark9.filterable.In,
        external_id=waymark9.filterable.Eq,
    )

    @action(from_=EntityState.ONBOARDING, to=EntityState.ONBOARDING,
            input=CrosswalkInput,
            edit=waymark9.Edit(prefill=("crosswalk_id",), fence=False,
...
async def env():
    ...
    CRM.pulls = 0
    CRM.discoverable = []
    engine = waymark9.Engine(resources=[Entity], storage=TEST_DSN,
    ...
    try:
        with pytest.raises(DefinitionError, match="authored"):
            await engine.invoker.invoke(
                ...
                principal=waymark9.Principal(id="dana"),
