...
import waymark9
from waymark9 import Ctx, DraftPolicy, Edit, Resource, Safety, action
from waymark9.client import AgentClient, mcp_tools
...
async def env():
    ...
    engine = waymark9.Engine(resources=[Note], storage=TEST_DSN,
...
async def test_state_graph_skips_non_envelope_docs():
    ...
    learner. Regression: `waymark9 client get …/-/grantable` raised
    KeyError('kind') because learn() assumed a resource envelope."""
    from waymark9.client.agent import StateGraph
    from waymark9.client.py import Doc
