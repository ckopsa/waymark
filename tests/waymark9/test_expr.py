...
import waymark9
from waymark9 import Acknowledged, Ctx, Derived, E, Resource, Safety, \
    action, guard
from waymark9.core.derived import derived_specs
from waymark9.core.expr import (
...
def test_x_derived_carries_the_expr_and_the_fingerprint_strips_it():
    from waymark9.core.fingerprint import _strip_derived_marks
...
async def test_guard_expr_advertises_and_enforces_one_tree():
    engine = waymark9.Engine(resources=[make_gated()], storage=TEST_DSN,
...
def test_count_and_sum_declare_expressions():
    ...
    class WireData(BaseModel):
        open_ifts: int = waymark9.Count(edge, where={"state": ("open",)})
        total: float = waymark9.Sum(edge, "amount")
