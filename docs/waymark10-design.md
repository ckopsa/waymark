...
| Collection shape | `data.items` negotiable: `depth=`, `rows=none`, declared columns | envelope-minus-data summaries; `depth=summary` and `rows=none` landed (batch A, §14); `fields=a,b` is the CALLER's column pick (waymark-pywy.2) — each item's `fields` narrowed out of the already grant-projected data, so it is always a subset of the grant's projection, an unknown name one 400 naming the caller's vocabulary; declared columns and `depth=expanded` profiles still absent |
