...
| 3 Thinking out loud | dry-run exists; transitions feed the ledger | **intent frames**: an agent's dry-runs/considerings broadcast ephemerally (presence-of-intent — same discipline as presence: ephemeral, never law, concealment-projected) |
| 4 Four hands | relay/2 + proven OT, per-field authors, stale never loses words, draft consumed by the act | **cursors/selections in the UI** (the named batch-D punt); author attribution needs identity over the WS (joins as anonymous today) |
| 5 The ask | warning walls, acknowledge-by-name, the guard's sentence identical in advertisement and refusal | **the asking surface**: an agent's pending warning/confirm surfaced to the human's screen as an addressed question (today it returns to the agent's channel; the human must be told out-of-band) |
