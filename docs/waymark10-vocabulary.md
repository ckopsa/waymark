...
| `:kind` | on a `:waymark/ref` entry: the target kind — the picker, the navigable link, and the assembly ref-check all read it. Three shapes may wear it: `:waymark/ref`, `[:maybe :waymark/ref]`, `[:vector :waymark/ref]`. `checks/check-ref-shape` refuses any other, because `:kind` says the field holds the id of a row (waymark-fp62.7.8). A LIST publishes the annotation twice: on the array, where the collection's filter param reads it, and on the ITEMS, where the form reads it — a list of refs is one picker per row, seeded id by id | `[:plan_id {:kind :plan} :waymark/ref]`, `[:held_for {:kind :model} [:vector :waymark/ref]]` |
...
| `:x-display` | advertisement: `{:label …}`, `{:help "…"}` (the hint sentence rendered under the field, and the one an MCP agent reads), `{:choices {"token" "the sentence a person reads"}}` (enum value prose — a select shows the sentence, the wire keeps the token), `{:widget "prose"}`, `{:hidden true}`, `{:when {:sibling "value"}}` (the field belongs to one of a sibling's choices and the form shows it only while that sibling holds the value — a hidden field keeps what was typed and is not submitted; the door's own guard still judges the pair), `{:showcase true}` — showcase renders a filterable field's filter as a standing control above the collection table (a select for enum'd/faceted fields, a min–max pair for `:range`, a date input for `:after`) instead of tucking it in the Filters popover | `[:kind {:x-display {:showcase true}} …]` |
| `:x-options` | advertisement of a RUNTIME vocabulary (`docs/spec-usability-battery.md`, waymark-8sg): `{:from source :of sibling :each bool :composes :query}` — where this field's legal tokens are enumerable from, as a one-hop fetch recipe the published schema carries. `:from` is a key of `schema/option-sources` (`:kinds`, `:fields`, `:actions`, `:views`, `:filters`, and `:places` — the one that answers out of a *row*: the sibling in `:of` holds an address and the recipe's href is that address with `/-/places` behind it, waymark-z8u4). A `{hole}` in the projected href or `at` is filled from the form in front of the person: **the bare name at the top of the form first, then — from inside a sub-form or a list row — the sub-form's own siblings by the last segment of their path**, so `launch.from` reads `{subject}` from the top and `scope[2].actions` reads `{kind}` from its own row; `checks/check-options` refuses an `:of` neither level declares. Advertisement, never law: the guard still judges, the fingerprint never moves | `[:from {:x-options {:from :places :of :subject}} …]`, `[:actions {:x-options {:from :actions :of :kind :each true}} …]` |
