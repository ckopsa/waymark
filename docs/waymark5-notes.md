...
| Anchor on the wire | `server/render.py` (`meta.law`) + `server/events.py` (`event_payload`) | envelopes and collections carry `meta.law`; one payload serializer feeds SSE and webhooks, so both carry `defined_by`; derivation events anchor to the law at emit. All additive — a v4 client keeps working |
...
- Framework tests: `tests/waymark5/` — the forked baseline plus
  `test_definition` (7), `test_anchoring` (8), `test_continuity` (5),
  `test_backfill` (7), `test_backfill_recovery` (6 — the crash window:
  durable marker, resumed drains, sort blocking, removed-fact markers,
  marker/revise atomicity): 259 passed.
