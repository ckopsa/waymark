(ns factory10.main
  ...
  and the tree that classifies it (bead waymark-fp62.6.2).
...
(defn resources
  ...
  domain. A `change` first, because a `ci_run` points at one."
  []
  (in-domain :factory [change ci-run]))

(defn check-resources
  "Zero-arg, so the declaration gate needs no env and no adapter.
  Both kinds are pull-only mirrors with no external seam in this
  bead — the source of waymark-fp62.6.4 writes them through the
  engine's own doors — so this is `resources` and nothing more."
  []
  (resources))
