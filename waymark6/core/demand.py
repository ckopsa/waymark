...
def demand_class(defn: ActionDef, schema: dict[str, Any] | None) -> str:
    ...
    Conditional demand (design §5): fields a ``When`` conditionally
    requires are excluded — the entry's ``effort`` reflects the *base*
    branch, the demand a caller can always meet, and the schema's
    ``if``/``then`` block is the honest advertisement of what the
    conditional arm will additionally ask. (Per-branch effort on the
    entry itself would put two numbers where clients expect one; the
    branch's own demand is derivable from the advertised ``then``.)"""
    ...
    from .when import conditional_fields

    conditional = conditional_fields(defn.input)
    ...
    for name, raw in props.items():
        ...
        if name in conditional:
            continue  # the conditional arm's demand; the base branch's isn't
