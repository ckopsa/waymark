...
class Guard:
    ...
    def __init__(
        ...
        *,
        ...
        # The raw declaration is kept (design §10): a CREATE guard's check
        # grades the input by definition (r=None), so the create-surface
        # probe runs a check only when the declaration explicitly said
        # needs_input=False — "my verdict decides without input".
        self.declared_needs_input = needs_input
...
def four_eyes(of: str, *, explain: str | None = None,
    ...
    """Authority conditioned on history (design E3, re-founded on §4):
    whoever performed the ``of`` transition on this resource cannot
    perform the guarded one.

    Sugar for the general mechanism — ``Unless(actor_of(of))`` — the
    E-wave's fate applied to E3: kept in behavior, generalized in
    mechanism. One declared log fact; the projector renders the
    performer's action honestly ``unavailable`` with this reason, and the
    invoker refuses with the identical sentence. The rule the mapping
    series found enforced by display-name string equality in one app and
    by nothing at all in two others."""
...
class FactRequired(Guard):
    """``require("all_ifts_terminal")`` (design §5): a guard over a
    declared boolean derived field — the parent-gated-on-children gate
    come off the prose ``check=`` cliff, because the rollup is an
    ordinary field.

    The verdict judges the *stored* fact — the maintained column the
    engine wrote in the same commit as whatever last dirtied it — so the
    projector's fold and the invoker's refusal read the same bytes; there
    is no second evaluation to drift. The refusal's reason is generated
    from the ``Derived`` spec's ``explain=``/``vars=`` (this guard is the
    consumer that carried metadata was declared for), never hand-written
    ...
    recomputed ad hoc — says when the fact can change, so the engine
    says so.
    """
    ...
    async def _check_fact(self, r: Any, inp: Any, ctx: Ctx) -> Allow | Deny:
        ...
        if spec is not None and spec.vars is not None:
            from .derived import resolve_inputs

            try:
                vars = dict(spec.vars(*await resolve_inputs(spec, r, ctx)))
            except Exception:
        ...
        if not self._own_explain:
            ...
            if spec is not None and spec.explain:
                return spec.explain.format_map(
                    _SafeDict(dict(deny.vars or {})))
        return super().render_reason(deny, r)

    def becomes_available(self, deny: Deny, r: Any = None) -> dict[str, Any] | None:
        hope = super().becomes_available(deny, r)
        if hope is not None or r is None:
            return hope
        spec = self._spec(r)
        if spec is None or not spec.has_clock:
            return None
        if spec.flips_at is not None:
            ...
            at = getattr(r, "next_flip_at", None)
        return {"at": at.isoformat()} if at is not None else None
