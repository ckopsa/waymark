...
- ``waymark6 migrate`` snapshots the registry's storage schema
...
- Revisions are plain ``NNNN_*.sql`` files under ``migrations/waymark6/``,
  applied in order and recorded in ``waymark6_schema_migrations`` —
...
MIGRATIONS_TABLE = "waymark6_schema_migrations"
...
def emit(storage: PostgresStorage, directory: Path, *,
    ...
    header = ("-- waymark6 migration: generated from the declared registry.\n"
...
async def apply(dsn: str, directory: Path) -> list[str]:
    ...
    ``waymark6_schema_migrations``. Refuses files with unresolved REVIEW
