...
- Engine tables are ``waymark6_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark6 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark6_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark6_transitions", self.metadata,
            ...
            Index("ix_waymark6_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark6_idempotency", self.metadata,
        ...
        self.webhook_cursors = Table(
            "waymark6_webhook_cursors", self.metadata,
        ...
        self.cursors = Table(
            "waymark6_cursors", self.metadata,
        ...
        self.job_leases = Table(
            "waymark6_job_leases", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark6 migrate``'s
