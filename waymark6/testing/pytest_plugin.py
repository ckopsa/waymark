"""Pytest plugin: ``pytest --waymark6`` runs the 2.0 conformance suite
...
- a ``waymark6_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm6``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark6") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark6.testing.conformance", "waymark6.testing.walker"):
        return
...
async def wm6(waymark6_engine):
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark6-conformance")
    try:
