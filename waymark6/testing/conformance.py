...
Collected when pytest runs with ``--waymark6``. Parametrized per resource ×
...
async def make(wm6: Any, kind: str, state: str) -> Any:
    try:
        instance = await reg.make_state(kind, state, wm6.engine)
...
async def fetch(wm6: Any, kind: str, id: str, pname: str) -> dict[str, Any]:
    rdef = wm6.registry[kind]
    res = await wm6.client.get(f"{wm6.base}/{rdef.plural}/{id}", headers=H(pname))
    assert res.status_code == 200, res.text
    return res.json()


async def build_input(wm6: Any, kind: str, defn: ActionDef,
    ...
    try:
        return await reg.synthesize_input(wm6.engine, wm6.registry[kind],
                                          defn, instance)
    except reg.SkipState as e:
        pytest.skip(str(e))
...
async def post(wm6: Any, doc: dict[str, Any], defn: ActionDef, pname: str,
    ...
    return await wm6.client.post(href, json=body, headers=headers)


async def storage_version(wm6: Any, kind: str, id: str) -> int:
    async with wm6.storage.session() as s:
        loaded = await wm6.storage.load(s, kind, id)
    return loaded.version


async def transition_count(wm6: Any) -> int:
    from sqlalchemy import func, select

    async with wm6.storage.session() as s:
        return (await s.execute(
            select(func.count()).select_from(wm6.storage.transitions))).scalar_one()


async def principals_with(wm6: Any, kind: str, instance_state: str,
    ...
    out = []
    for pname in sorted(wm6.principals):
        instance = await make(wm6, kind, instance_state)
        doc = await fetch(wm6, kind, instance.id, pname)
...
async def test_representation(wm6, wm3_case):
    ...
    schema_res = await wm6.client.get(f"{wm6.base}/schemas/{kind}")
    assert schema_res.status_code == 200
    jsonschema.validate(doc["data"], schema_res.json())

    assert doc["meta"]["version"] == await storage_version(wm6, kind, instance.id)
...
async def test_affordance_completeness(wm6, wm3_case):
    kind, state, pname = wm3_case
    rdef: ResourceDef = wm6.registry[kind]
    instance = await make(wm6, kind, state)
    doc = await fetch(wm6, kind, instance.id, pname)
    ...
    for name in hidden:
        ...
        body = await build_input(wm6, kind, defn, instance)
        res = await post(wm6, doc, defn, pname, body)
...
async def test_token_prose(wm6, wm3_case):
...
async def test_orientation(wm6, wm3_kind):
    ...
    for state in rdef.machine.states:
        try:
            ...
            continue
        doc = await fetch(wm6, kind, instance.id, sorted(wm6.principals)[0])
...
# ── Transition truth ────────────────────────────────────────────────────
async def test_transition_truth_available(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm6.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm6, kind, state, action, "actions")
    if not cases:
    ...
    for pname, instance, doc in cases:
        ...
        async with wm6.storage.session() as s:
            last = await wm6.storage.last_transition(s, kind, instance.id)
        assert last.action == action
        assert last.from_state == state and last.to_state == defn.to
        assert last.actor_id == wm6.principals[pname].id
        assert last.actor_type == wm6.principals[pname].type

        refetched = await fetch(wm6, kind, instance.id, pname)
        for field in ("state", "data", "meta", "actions"):
            assert refetched[field] == after[field], \
                "response is not the post-transition document"


async def test_transition_truth_unavailable(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm6.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm6, kind, state, action, "unavailable")
    if not cases:
...
async def test_touch_truth(wm6, wm3_action_case):
    ...
    async with wm6.storage.session() as s:
        root = await wm6.storage.last_transition(s, kind, instance.id)
        story = await wm6.storage.transitions_by_correlation(
            s, root.correlation_id)
    actor = wm6.principals[pname].id
    ...
    for t in defn.touches:
        if isinstance(t, Creates):
            created = wm6.registry.get(t.kind)
...
# ── Safety truth ────────────────────────────────────────────────────────
async def test_safety_idempotent_double_invoke(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm6.registry[kind].machine.actions[action]
    if not defn.safety.idempotent:
    ...
    pname, instance, doc = cases[0]
    body = await build_input(wm6, kind, defn, instance)
    first = await post(wm6, doc, defn, pname, body)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200
    v1 = first.json()["meta"]["version"]

    second = await post(wm6, doc, defn, pname, body,
...
async def test_safety_non_idempotent_key_discipline(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm6.registry[kind].machine.actions[action]
    ...
    key = uuid.uuid4().hex
    first = await post(wm6, doc, defn, pname, body, idem_key=key)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200, first.text

    replay = await post(wm6, doc, defn, pname, body, idem_key=key)
    ...
    if defn.input is not None:
        ...
        reuse = await post(wm6, doc, defn, pname, mutated, idem_key=key)
        assert reuse.status_code == 409, \
            f"key reuse with different body must 409, got {reuse.status_code}"


async def test_safety_if_match(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm6.registry[kind].machine.actions[action]
    ...
    pname, instance, doc = cases[0]
    body = await build_input(wm6, kind, defn, instance)
    stale = await post(wm6, doc, defn, pname, body, etag='W/"conformance-stale"')
...
async def test_safety_reversible(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm6.registry[kind].machine.actions[action]
...
# ── Input contract ──────────────────────────────────────────────────────
async def test_input_contract(wm6, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm6.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm6, kind, state, action, "actions")
    ...
    if defn.input is not None:
        junk = await post(wm6, doc, defn, pname, {"__conformance_bogus__": 1})
    ...
    body = await build_input(wm6, kind, defn, instance)
    v_before = await storage_version(wm6, kind, instance.id)
    t_before = await transition_count(wm6)
    dry = await post(wm6, doc, defn, pname, body, dry_run=True)
    ...
    assert await storage_version(wm6, kind, instance.id) == v_before, \
        "dry_run changed the resource version"
    assert await transition_count(wm6) == t_before, \
...
async def test_schema_guard_gap(wm6, wm3_action_case):
    ...
    async with wm6.storage.session() as s:
        ctx = wm6.engine.invoker._ctx(wm6.principals[pname], s, mode="dry_run")
        for g in derivable:
            denies = 0
            for inp in parsed:
...
async def test_prefill_truth(wm6, wm3_action_case):
...
async def test_draft_protection(wm6, wm3_action_case):
    ...
    href = advert["href"]
    empty = await wm6.client.get(href, headers=H(pname))
    ...
    body = await build_input(wm6, kind, defn, instance)
    res = await wm6.client.put(href, json=body, headers=H(pname))
    assert res.status_code == 200, res.text
    saved = res.json()
    ...
    fresh = await wm6.client.get(href, headers=H(pname))
    ...
    for other in sorted(wm6.principals):
        if other == pname:
            continue
        other_get = await wm6.client.get(href, headers=H(other))
    ...
    junk = await wm6.client.put(href, json={"__not_a_field__": 1},
                                headers=H(pname))
    assert junk.status_code == 422, \
        "draft fields outside the action's schema must be rejected"

    res = await post(wm6, refetched, defn, pname, body)
    fail_if_needs_example(res, kind, defn)
    assert res.status_code == 200, res.text
    after_get = await wm6.client.get(href, headers=H(pname))
...
# ── Collection contract ─────────────────────────────────────────────────
async def test_collection_contract(wm6, wm3_kind):
    kind = wm3_kind
    rdef = wm6.registry[kind]
    cls = rdef.cls
    pname = sorted(wm6.principals)[0]
    base = f"{wm6.base}/{rdef.plural}"

    async def query(qs: str) -> dict:
        res = await wm6.client.get(f"{base}?{qs}", headers=H(pname))
        assert res.status_code == 200, res.text
        return res.json()
    ...
    for state in cls.__waymark_machine__.states:
        try:
            made.append(await reg.make_state(kind, state, wm6.engine))
        except reg.SkipState:
            continue
    if not made:
    ...
    while True:
        ...
        if not nxt:
            break
        res = await wm6.client.get(nxt["href"], headers=H(pname))
        assert res.status_code == 200, res.text
        doc = res.json()
...
async def test_bulk_report(wm6, wm3_kind):
    ...
    for defn in bulk_actions:
        ...
        body = dict(await build_input(wm6, kind, defn) or {})
        body["ids"] = ids
        headers = H(sorted(wm6.principals)[0])
        if not defn.safety.idempotent:
            headers["Idempotency-Key"] = uuid.uuid4().hex
        res = await wm6.client.post(f"{wm6.base}/{rdef.plural}/-/{defn.name}",
...
# ── Events ──────────────────────────────────────────────────────────────
async def test_events(wm6, wm3_kind):
    ...
    rdef = wm6.registry[kind]
    machine = rdef.cls.__waymark_machine__
    instance = await make(wm6, kind, machine.initial)
    ...
    sub = wm6.engine.dispatcher.subscribe(resource=(kind, instance.id))
    try:
        body = await build_input(wm6, kind, defn, instance)
        res = await post(wm6, chosen_doc, defn, principal, body)
        fail_if_needs_example(res, kind, defn)
        assert res.status_code == 200, res.text
        after = res.json()
...
async def test_replay_history(wm6, wm3_kind):
    ...
    the current law."""
    kind = wm3_kind
    machine = wm6.registry[kind].cls.__waymark_machine__
    ...
    pre = await _all_transitions(wm6.storage, kind)
    horizon = pre[-1].id if pre else 0
    made = 0
    for state in machine.states:
        try:
            await reg.make_state(kind, state, wm6.engine)
            made += 1
        except reg.SkipState:
            continue
    if not made:
        pytest.skip("no states producible")
    fresh = [t for t in await _all_transitions(wm6.storage, kind)
             if t.id > horizon]
    assert fresh, "the walk wrote no history to replay"
    law = wm6.engine.current_law(kind)
    assert law is not None
    assert all(t.defined_by == law for t in fresh), \
        "a write under this boot must anchor to this boot's law"
    checked = await replay_history(wm6.storage, wm6.registry, kind)
    assert checked >= len(fresh)


async def test_log_prose_never_rerendered(wm6, wm3_kind):
    ...
    kind = wm3_kind
    machine = wm6.registry[kind].cls.__waymark_machine__
    await make(wm6, kind, machine.initial)
    rows = await _all_transitions(wm6.storage, kind)
    assert rows, "creating an instance must have logged a transition"
    for t in rows:
        payload = event_payload(t, wm6.registry, wm6.base)
        ...
        again = event_payload(t, wm6.registry, wm6.base)
...
async def test_migration_roundtrip(wm6):
    ...
    snapshot = wm6.storage.schema_snapshot()
    stmts = m.diff_statements(wm6.storage, None)

    scratch = "waymark6_conformance_migration"
    async with wm6.storage.engine.begin() as conn:
...
# ── Suite integrity ─────────────────────────────────────────────────────
async def test_every_resource_is_enrolled(wm6):
    missing = [k for k in wm6.registry.kinds() if k not in reg.resources()]
    assert not missing, (
