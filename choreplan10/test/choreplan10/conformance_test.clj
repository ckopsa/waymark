(ns choreplan10.conformance-test
  "Both kinds enrolled in the waymark10 conformance library: the
  ...
  real ring handler. Mirrors eveningplan10.conformance-test's shape;
  ...
  - no state factories: neither kind gates a transition behind a
    guard or a :while-open field, so the generic shortest-path walk
...
;; enums come from an acceptance-set :accepts guard; none of this
;; app's actions are that shape, so there's nothing for that check to
;; exercise)
