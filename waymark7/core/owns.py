...
class Never:
    """The seed retro-propagation policy that ships (design §4): template
    edits never retro-propagate. A revised seed template governs future
    instantiations only; children already created keep the values they
    were instantiated with, under the law that made them. E4 recorded
    this as prose in a notes file; here it is a declaration — it rides
    the fingerprint, so the policy has a diff class and a revise that
    touched the template names what did (and did not) change meaning.
    Other policies (recompute, re-seed) are a recorded punt: retro-
    propagation itself is not built."""

    def __init__(self) -> None:  # pragma: no cover - the class IS the token
        raise TypeError("Never is a policy token; use the class itself in "
                        "retro=")
...
class Seed:
    ...
    engine adds the edge's ``via`` itself. ``retro`` declares what a
    template edit means for existing children — :class:`Never` (the only
    shipped policy, design §4) says it means nothing."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.retro is not Never:
            raise DefinitionError(
                "Seed(retro=...) — Never is the only shipped retro policy "
                "(design §4); retro-propagation itself is a recorded punt")
