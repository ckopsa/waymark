...
- ``waymark7 migrate`` snapshots the registry's storage schema
...
- Revisions are plain ``NNNN_*.sql`` files under ``migrations/waymark7/``,
  applied in order and recorded in ``waymark7_schema_migrations`` —
...
MIGRATIONS_TABLE = "waymark7_schema_migrations"
...
def emit(storage: PostgresStorage, directory: Path, *,
    ...
    header = ("-- waymark7 migration: generated from the declared registry.\n"
...
async def apply(dsn: str, directory: Path) -> list[str]:
    ...
    ``waymark7_schema_migrations``. Refuses files with unresolved REVIEW
