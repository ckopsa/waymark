...
def parse_query(rdef: ResourceDef, params: Any) -> dict[str, Any]:
    ...
    lists become IN filters where the field declares the In op. A param
    naming a fact still catching up with its redefinition (design §4's
    Deferred backfill) is refused, not answered from mixed-law values —
    the enforcement half of the collection schema's un-advertising."""
    from ..core.resource import FilterOp
    from ..core.schemas import field_params

    props = rdef.query_schema["properties"]
    recomputing = {p for fact in rdef.recomputing for p in field_params(fact)}
    parsed: dict[str, Any] = {}
    ...
    for name, raw in params.items():
        if name in ("depth", "dry_run", "part"):
            continue
        if name in recomputing:
            fact = next(f for f in rdef.recomputing
                        if name in field_params(f))
            raise FactRecomputing(
                f"The fact {fact!r} was redefined by the current deploy "
                "and its stored values are recomputing; filtering on it "
                "is temporarily unavailable until the backfill completes.",
                recomputing=sorted(rdef.recomputing))
        if name == "sort" and str(raw).removeprefix("-") in rdef.recomputing:
            # sorting is blocked with the filters (design §4): an order
            # over a half-recomputed column ranks rows by two laws at
            # once — refused with the same Problem the filter gets, and
            # un-advertised from the sort enum by the same render
            fact = str(raw).removeprefix("-")
            raise FactRecomputing(
                f"The fact {fact!r} was redefined by the current deploy "
                "and its stored values are recomputing; sorting on it "
                "is temporarily unavailable until the backfill completes.",
                recomputing=sorted(rdef.recomputing))
        ...
        try:
            ...
            if schema.get("type") == "integer":
                ...
                # JSON spellings only; Python's str(True) is a malformed
                # value, and malformed values are Problems (design §7)
                ...
                # temporal params (design 6.0 §3): the advertised format is
                # enforced here — a malformed boundary is a per-field 422,
                # never a float-parse error or a driver surprise. Values
                # stay ISO strings; the storage layer binds them to the
                # promoted column's type.
                ops = fspec.fields.get(name) if fspec else None
                parts = (raw.split(",") if "," in raw and ops
                         and ops & FilterOp.IN else [raw])
                for part in parts:
                    _parse_temporal(part, schema["format"])
                if len(parts) > 1:
                    value = parts
            elif schema.get("type") == "string":
                ops = fspec.fields.get(name) if fspec else None
...
def _parse_temporal(raw: str, fmt: str) -> None:
    """Validate one advertised-temporal param value (design 6.0 §3):
    ``date`` params are ISO 8601 dates, ``date-time`` params RFC 3339
    timestamps. Raises ``ValueError`` with the sentence the per-field
    422 carries."""
    ...
    try:
        ...
        expected = ("an ISO 8601 date (YYYY-MM-DD)" if fmt == "date"
                    else "an RFC 3339 date-time")
        raise ValueError(f"must be {expected}") from None


def _stream_classes(request: Any) -> frozenset[str]:
    """The event classes a stream subscriber receives (design §3):
    transitions and derivation flips by default; ``?classes=`` narrows —
    a v3 client that only wants writes says ``classes=transition``."""
    raw = request.query_params.get("classes")
    if not raw:
        return frozenset({"transition", "derivation"})
...
def split_query(rdef: ResourceDef, parsed: dict[str, Any]) -> tuple[dict, str | None, int, int]:
    filters = {k: v for k, v in parsed.items()
               if k not in ("sort", "page[size]", "page[number]", "rows")}
    sort = parsed.get("sort")
    if sort is None and rdef.cls.sortable is not None:
        sort = rdef.cls.sortable.default
        if sort is not None and sort.removeprefix("-") in rdef.recomputing:
            # a declared default naming a recomputing fact would order by
            # mixed-law values nobody asked for; serve unordered instead
            sort = None
...
def build_router(engine: Any) -> APIRouter:
    ...
    async def wellknown() -> Response:
        doc = {
            ...
            **({"attachment_bytes":
            ...
            # with the attention filter the dashboard strip renders from —
            # nothing inferred that a declaration could have said
            "surfaces": {
                name: {
                    "anchor": sdef.anchor.kind,
                    "href": f"{base}/surfaces/{name}/{{anchor_id}}",
                    "collection": f"{base}/{sdef.anchor.plural}",
                    **({"attention": sdef.attention}
                       if sdef.attention else {}),
                }
                for name, sdef in sorted(engine.surfaces.items())},
        }
    ...
    async def firehose(request: Request) -> Any:
        ...
        # recorded inputs (design 7.0 §5) ride the stream for
        # full-visibility principals only — the stream is not projected
        # per row, so full-only is the honest cut (recorded deviation)
        full = getattr(visibility_of(principal, _now()), "full", False)
    ...
    async def _enqueue_approval(request: Request, grant: Any,
        ...
        create_body = {
            ...
            "missing": missing,
            # warning-guard names the agent acknowledged on its request:
            # recorded on the approval (the human reviews them with the
            # input) and carried through the approved run (design §10)
            "acknowledged": sorted(_acknowledged(request)),
        }
    ...
    async def resource_events(plural: str, id: str, request: Request) -> Any:
        ...
        sub = engine.dispatcher.subscribe(
            ...
            classes=_stream_classes(request),
            paused=bool(request.headers.get("Last-Event-ID")))
        # recorded inputs (design 7.0 §5): full-visibility principals only
        # — see the firehose route's note; the same recorded deviation
        full = getattr(visibility_of(principal, _now()), "full", False)
        return sse_response(sse_stream(
            engine.dispatcher, sub, registry, base,
            last_event_id=request.headers.get("Last-Event-ID"),
    ...
    async def _collection_doc(rdef: ResourceDef, params: Any,
                              principal: Principal) -> dict[str, Any]:
        """The collection envelope for one set of query params (anything
        with ``.items()`` — a request's query_params or a dict a surface
        member compiled from its link). One code path: the same grammar,
        the same visibility pushdown, the same rendering."""
        from .grants import visibility_of

        parsed = parse_query(rdef, params)
        # ?rows=none (design §9): the envelope — total, facets, the query
        # action — with no items; the count runs the same WHERE the rows
        # would, so the number in the tab cannot disagree with the grid
        rows_none = parsed.get("rows") == "none"
        ...
        async with engine.storage.session() as s:
            items, total = await engine.storage.query(
                ...
                restrict=restrict, include_rows=not rows_none)
            facets = None
            # a recomputing fact's counts would mix two laws (design §4):
            # its facet is un-advertised with its filter until caught up
            faceted = [f for f in ("state", *rdef.cls.faceted)
                       if rdef.cls.filterable
                       and f in rdef.cls.filterable.fields
                       and f not in rdef.recomputing]
            if faceted:
    ...
    async def lookup_collection(plural: str, request: Request) -> Response:
        ...
        doc = await _collection_doc(rdef, request.query_params, principal)
        return Response(content=json.dumps(doc, default=str),
                        media_type=MEDIA_TYPE)

    # ── the decision surface (design 6.0 §4): served as an ordinary
    # envelope whose data is the anchor's, whose declared members arrive
    # embedded, and whose showcased affordances render first. Registered
    # before the generic /{plural} routes so the fixed segment wins. ────
    async def _member_embed(sdef: Any, m: Any, instance: Any,
                            principal: Principal,
                            anchor_doc: dict[str, Any]) -> dict[str, Any]:
        """One member, embedded: the link object plus the rows its href
        selects, fetched through the SAME query path the collection route
        runs — visibility pushdown included, no second authorization."""
        from ..core.summary import _SummaryFormatter
        from .grants import visibility_of
        from .render import render

        href = _SummaryFormatter(instance).vformat(m.link.href, (), {})
        link_entry = dict(anchor_doc.get("links", {}).get(m.rel) or {
            "href": base + href if href.startswith("/") else href,
            "kind": m.link.kind})
        link_entry.pop("embedded", None)
        entry: dict[str, Any] = {**link_entry, "items": [], "total": 0}
        if m.table:
            # which columns matter is declared intent, not data filtering:
            # visibility still governs actual field presence (design §4)
            entry["x-display"] = {"columns": list(m.table)}
        path, _, query = href.partition("?")
        segments = [p for p in path.strip("/").split("/") if p]
        try:
            if len(segments) == 2 and segments[0] == m.target.plural:
                # a to-one link: the member is the single referenced row,
                # held to the same restriction the collection WHERE applies
                async with engine.storage.session() as s:
                    inst = await engine.storage.load(s, m.target.kind,
                                                     segments[1])
                    vis = visibility_of(principal, _now())
                    restrict = vis.restrict(m.target.kind)
                    if inst is not None and restrict is not None:
                        owner_id, ids = restrict
                        if inst.id not in ids \
                                and (inst.owner or None) != owner_id:
                            inst = None
                    if inst is not None:
                        ctx = engine.invoker._ctx(principal, s, mode="probe")
                        entry["items"] = [await render(
                            inst, m.target, ctx=ctx, depth="summary",
                            base=base)]
                        entry["total"] = 1
            else:
                ...
                doc = await _collection_doc(m.target, params, principal)
                entry["items"] = doc["data"]["items"]
                entry["total"] = doc["data"]["total"]
        except Problem as exc:
            # a member that cannot resolve (a null join boundary, a
            # hand-templated typo) degrades honestly instead of failing
            # the whole surface — the client renders the sentence
            entry["problem"] = {"title": exc.title, "detail": exc.detail}
        return entry

    @router.get("/surfaces/{name}/{anchor_id}")
    @_wire
    async def surface(name: str, anchor_id: str,
                      request: Request) -> Response:
        from ..core.summary import _SummaryFormatter
        from .grants import visibility_of

        sdef = engine.surfaces.get(name)
        if sdef is None:
            raise NotFound(f"No surface {name!r}.")
        principal = await engine.resolve_principal(request)
        rdef = sdef.anchor
        async with engine.storage.session() as s:
            instance = await engine.storage.load(s, rdef.kind, anchor_id)
        if instance is None:
            raise NotFound(f"No {rdef.kind} {anchor_id!r}.")
        vis = visibility_of(principal, _now())
        if not vis.full and vis.action(
                sdef.target_kind, "view", anchor_id,
                owner=instance.owner) != "open":
            # the composed view is the grant target (design §4): a grant
            # naming surface:{name} (action `view`) opens the route —
            # ownership of the anchor opens it too (owner= threads the
            # standing "full over what you own" rule) — and member
            # visibility still applies per-kind underneath
            raise Forbidden(SCOPE_REASON,
                            action_attempted=f"{sdef.target_kind}.view",
                            remedies=["grant.request_access"])
        async with engine.storage.session() as s:
            ctx = engine.invoker._ctx(principal, s, mode="probe")
            doc = await engine.render_with_depth(s, instance, rdef,
                                                 ctx=ctx, depth="full")
        embedded = {m.rel: await _member_embed(sdef, m, instance,
                                               principal, doc)
                    for m in sdef.members}
        # showcased affordances render first (design §4): showcase-then-
        # declared order, with the flag the client can honor
        actions: dict[str, Any] = {}
        for aname in sdef.showcase:
            if aname in doc["actions"]:
                actions[aname] = {**doc["actions"][aname],
                                  "showcased": True}
        for aname, a_entry in doc["actions"].items():
            actions.setdefault(aname, a_entry)
        links = {rel: l for rel, l in doc["links"].items()
                 if rel != f"surface:{name}"}
        links["anchor"] = {"href": doc["self"], "kind": rdef.kind,
                           "summary": f"The {rdef.kind.replace('_', ' ')} "
                                      "this surface composes"}
        out = dict(doc)
        out["kind"] = sdef.target_kind
        out["self"] = f"{base}/surfaces/{name}/{anchor_id}"
        out["actions"] = actions
        out["links"] = links
        out["embedded"] = embedded
        if sdef.title and "display" in out:
            # scoped views dropped display with the anchor render — the
            # title template formats over fields the projection may not
            # admit, so it rides only the full view (the render rule)
            shim = type("A", (), {"anchor": instance})()
            out["display"] = {**out["display"],
                              "title": _SummaryFormatter(shim).vformat(
                                  sdef.title, (), {})}
        # meta: the anchor's etag/version, the SURFACE's own law (design
        # §4: what the reviewer saw is answerable under its own revision)
        meta = {k: v for k, v in doc["meta"].items()
                if k not in ("law", "law_revision")}
        if sdef.current_law:
            meta["law"] = sdef.current_law
        if sdef.current_law_revision:
            meta["law_revision"] = sdef.current_law_revision
        out["meta"] = meta
        await _announce("viewed", principal, sdef.target_kind, out["self"])
        return Response(content=json.dumps(out, default=str),
                        media_type=MEDIA_TYPE,
    ...
    async def _resource_doc(rdef: ResourceDef, id: str, request: Request,
        ...
        if getattr(rdef.cls, "adapter", None) is not None:
            # a resource with external truth past its TTL asks the outside
            # world first (design §10/§8): whole-resource Mirror or the
            # authored subset — an observed change lands as a system-actor
            # transition either way
            from .external import refresh_external

            instance = await refresh_external(engine, rdef, instance)
        async with engine.storage.session() as s:
            ctx = engine.invoker._ctx(principal, s, mode="probe")
    ...
    @router.post("/{plural}/{id}/-/{action}/batch")
    @_wire
    async def batch(plural: str, id: str, action: str,
                    request: Request) -> Response:
        """The batch form of a declared action (design §7): one generic
        route, like bulk's — the invoker refuses actions that declared no
        Batch. Scope gating mirrors the act route's projection: what did
        not render open cannot be batch-invoked."""
        rdef = rdef_or_404(plural)
        principal = await engine.resolve_principal(request)
        body = await _json_body(request)
        dry_run = request.query_params.get("dry_run") in ("1", "true")
        ...
        if member_vis is not None and _scope(principal) is None \
            ...
            if target is not None and member_vis.action(
                    rdef.kind, action, id, owner=target.owner) != "open":
        ...
        if grant is not None and rdef.machine.actions.get(action) is not None:
            ...
            if GrantVisibility(grant, _now()).action(
                    rdef.kind, action, id) != "open":
                # approval capture of whole batches is a recorded punt:
                # an approval-mode grant cannot batch-invoke
                raise Forbidden(SCOPE_REASON, action_attempted=action,
                                remedies=["grant.request_access"])
        if dry_run:
        ...
        result = await engine.invoker.batch(
            rdef.kind, id, action, body, principal=principal,
            idempotency_key=request.headers.get("Idempotency-Key"),
            dry_run=dry_run, acknowledged=_acknowledged(request))
        return _doc_response(result)
    ...
    async def _save_draft(plural: str, id: str, action: str,
        ...
        if defn.batch_spec is not None:
            # a batch draft stages the collection form (design §7): the
            # staged items dry-run to per-item verdicts at the batch route
            known.add("items")
