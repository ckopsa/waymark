"""Events (§6): the transition log is the outbox is the activity feed.

A single dispatcher task tails ``waymark_transitions`` — woken by Postgres
LISTEN/NOTIFY (sent transactionally in ``append_transition``) with a polling
...
log = logging.getLogger("waymark.events")
...
class Dispatcher:
    ...
    async def start(self) -> None:
        ...
        await self._try_listen()
        self._task = asyncio.create_task(self._run(), name="waymark-dispatcher")

    async def _try_listen(self) -> None:
        """LISTEN wakes us instantly; failure just leaves the polling fallback."""
        try:
            import asyncpg

            dsn = (self.storage.engine.url.set(drivername="postgresql")
                   .render_as_string(hide_password=False))
            self._listen_conn = await asyncpg.connect(dsn)
            await self._listen_conn.add_listener(
                NOTIFY_CHANNEL, lambda *_: self._wake.set())
        except Exception as exc:  # pragma: no cover - depends on environment
            log.warning("LISTEN unavailable, falling back to polling: %s", exc)
