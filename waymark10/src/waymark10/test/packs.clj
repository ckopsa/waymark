(ns waymark10.test.packs
  ...
  The module namespaces required below (attachments, belief, curtain,
...
;; ── the belief layer ────────────────────────────────────────────────
;;
;; The hypothesis obligation rode the feed's pack until the feed was
;; retired (2026-09). It never read the feed: a hypothesis is a claim
;; with a number on it, folded from typed findings, and the pack below
;; proves the number from the wire.
...
(defn- leash!
  ...
  ;; kind: a request presents exactly one X-Waymark-Grant.
  ([ctx audience scope]
...
(defn- hypothesis-violations
  ...
  any other finding's rows — and it is the arm the backfill re-pass
  ...
  (let [hs (finder-headers)
        ...
        v (cond-> []
            ...
            (conj (str "belief: a create body naming posterior and"
            ...
            (conj (str "belief: the refusal named " (pr-str stray-errors)
            ...
            (not= 201 (:status born))
            (conj (str "belief: a reading could not write down what it noticed ("
    ...
    (if (nil? id)
      ...
      (let [d (:doc born)
            ...
            v (cond-> v
                ...
                (conj (str "belief: a second intent belief about the same row"
                ...
                (conj (str "belief: not-a-second-belief did not name the"
            ...
            v (cond-> v
                (not= 201 (:status atom'))
                (conj (str "belief: a typed finding could not cite a belief"
            ...
            v (cond-> v
                ...
                (conj (str "belief: a belief reports "
                ...
                (conj (str "belief: the posterior did not move when the atom"
                           " landed (" (pr-str p0) " to "
            ...
            v (cond-> v
                ...
                (conj (str "belief: the belief fold failed ("
                           (pr-str swept) ")"))

                (or (nil? p1) (nil? p0) (<= p1 p0))
                (conj (str "belief: an unprompted mention did not lift the"
                ...
                (conj (str "belief: the belief reports "
                ...
                (conj (str "belief: the cached atom does not name the finding"
                           " it came from ("
                ...
                (conj (str "belief: the cached atom does not say which of the"
                           " nine words it was ("
                           (pr-str (get-in after [:data :atoms 0])) ")"))
                ...
                (not= 8.0M
                      (some-> (get-in after [:data :atoms 0 :lr_applied])
                              bigdec (.setScale 1)))
                (conj (str "belief: the cached atom is priced at "
                ...
                (conj (str "belief: the belief moved and says nothing about"
            ...
            v (cond-> v
                ...
                (conj (str "belief: a reading could not correct its own"
                           " wording (" (:status reworded) " "
                           (pr-str rdoc) ")"))

                (not= [subject] (mapv str (get-in rdoc [:data :about])))
                (conj (str "belief: restating with the claim alone left the"
                           " belief about "
                ...
                (conj (str "belief: restating with the claim alone left "
                ...
                (or (nil? p1') (nil? p0) (<= p1' p0))
                (conj (str "belief: correcting the words dropped the belief"
            ...
            v (cond-> v
                ...
                (conj (str "belief: a restate that empties `about` was not"
                ...
                (conj (str "belief: a restate that drops the only address a"
                ...
                (conj (str "belief: the narrowing restate warned "
                           (pr-str warned)
                ...
                (not= 1 (get-in (json ctx (get-env ctx :hypothesis id))
                                [:data :atom_count]))
                (conj (str "belief: an unacknowledged warning changed the"
            ...
            v (cond-> v
                (nil? leashed)
                (conj (str "belief: could not leash the reading over"
                ...
                (conj (str "belief: the reading that noticed a belief"
                ...
                (conj (str "belief: the four-eyes refusal named "
            ...
            v (cond-> v
                ...
                (conj (str "belief: a second hand could not affirm the belief ("
                           (:status answered) " " (pr-str adoc) ")"))

                (not= "affirmed" (str (:state adoc)))
                (conj (str "belief: an affirmed belief rests in "
                ...
                (or (nil? p2) (> (Math/abs (- p2 (double (or p1' p1 0.0))))
                                 0.001))
                (conj (str "belief: affirming moved the number ("
...
(def belief
  {:module :belief
   :obligations
   [{:name :belief/hypotheses
     :needs #{[:kind :hypothesis] [:kind :insight]}
     :run hypothesis-violations}]})
