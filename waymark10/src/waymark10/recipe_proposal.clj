...
(defhandler apply-the-order
  ...
  (let [d (:data row)
        ...
        ;; …and the findings' six, the same way (waymark-1uv.8's own
        ;; hazard, waymark-bck): until a proposal can name them, an
        ;; order-only apply must not reset them to the deployment's
        findings (when (and tid (:read ctx))
                   (get-in ((:read ctx) :feed_recipe tid)
                           [:data :insight_rank]))
  ...
  {:proposal
   ...
   {:examples [feed-recipe/order-example]
    ...
    {:label "The order the house reads today"
     :spelled-by-hand "A list of lines is a list of maps, and a list of maps can't list another kind's field names, so each line is typed by hand: the example is the whole shape — copy the current order and edit a line; the chips beside the box offer every section and population word."
