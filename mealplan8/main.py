"""The family meal-planning app: resource definitions in, everything else out.
    ...
    uv run uvicorn mealplan8.main:app --reload
...
def _dsn() -> str:
    ...
    (one KEY=VALUE per line) so `uv run uvicorn mealplan8.main:app` works
    ...
    # 2.0 tables differ from v1's (waymark8_* engine tables, timestamptz
    # generated columns) — default to a fresh database rather than colliding
    # with a v1 deployment's schema
    return "postgresql+asyncpg://localhost/mealplan8_dev"
...
from waymark8.server import openapi as _openapi  # noqa: E402

_openapi.install(app, engine.registry)

# The server hands clients their client: built wheels (waymark8 CLI) are
...
if os.path.isdir(CLI_DIR):
    ...
    def cli_index(request: Request) -> PlainTextResponse:
        ...
        lines = ["# waymark8 client — install with one of:", ""]
        for wheel in wheels:
            lines += [f"uv tool install {base}/cli/{wheel}",
                      f"pipx install {base}/cli/{wheel}"]
        lines += ["", f"# then: WAYMARK_BASE={base} waymark8 client index",
