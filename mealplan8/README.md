# The family meal planner (waymark8)

The meal planner on Waymark 7.0 — the mealplan6 app forked onto the
"law binds rows" runtime (`docs/waymark8-design.md`). The calendar
...
```bash
make mealplan8                               # db + wheel + uvicorn on :8005
...
This app runs on **waymark8** (see `docs/waymark8-design.md`). Meal,
...
```bash
uv run waymark8 check mealplan8.main:engine
make conformance7                     # pytest --waymark8 -n auto
uv run pytest --waymark8 plan         # or meal, event, rotation, …
```
