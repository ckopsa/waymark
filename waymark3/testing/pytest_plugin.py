"""Pytest plugin: ``pytest --waymark3`` runs the 2.0 conformance suite
...
- a ``waymark3_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm3``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark3") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark3.testing.conformance", "waymark3.testing.walker"):
        return
...
async def wm3(waymark3_engine):
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark3-conformance")
    try:
