"""--waymark3-walk: random-walk each resource's *real* transitions from its
    ...
    for walk in range(WALKS):
        ...
        for step in range(MAX_STEPS):
            ...
            async with wm3.storage.session() as s:
                current = await wm3.storage.load(s, kind, instance.id)
            body = await build_input(wm3, kind, defn, current)
            headers = H(pname)
            if defn.safety.fence:
                headers["If-Match"] = doc["meta"]["etag"]
            if not defn.safety.idempotent:
                headers["Idempotency-Key"] = uuid.uuid4().hex
            res = await wm3.client.post(f"{doc['self']}/-/{action}",
