"""Pytest plugin: ``pytest --waymark5`` runs the 2.0 conformance suite
...
- a ``waymark5_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm5``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark5") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark5.testing.conformance", "waymark5.testing.walker"):
        return
...
async def wm5(waymark5_engine):
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark5-conformance")
    try:
