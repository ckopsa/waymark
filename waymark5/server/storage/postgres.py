...
- Engine tables are ``waymark5_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark5 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark5_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark5_transitions", self.metadata,
            ...
            Index("ix_waymark5_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark5_idempotency", self.metadata,
        ...
        self.webhook_cursors = Table(
            "waymark5_webhook_cursors", self.metadata,
        ...
        self.cursors = Table(
            "waymark5_cursors", self.metadata,
        ...
        self.job_leases = Table(
            "waymark5_job_leases", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark5 migrate``'s
