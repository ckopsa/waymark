...
class ApprovalData(BaseModel):
    ...
    acknowledged: list[str] = Field(
        default_factory=list,
        description="Warning-guard names the requester already acknowledged; "
                    "they carry through the approved run (design §10)")
    ...
    async def run(self, inp: None, ctx: Ctx) -> None:
        ...
        try:
            if self.data.target_action == "create":
                ...
                # and it carries the recorded acknowledgments like any
                # ordinary write (design §10): the human approved them
                # alongside the input
                doc = await ctx.create(
                    self.data.target_kind, self.data.target_input or {},
                    acknowledged=frozenset(self.data.acknowledged))
...
class GrantVisibility:
    """The grant-backed :class:`~waymark5.core.visibility.Visibility`
