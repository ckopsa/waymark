# The family meal planner (waymark6)

The meal planner on Waymark 6.0 — the dogfood for the design's driving
story (`docs/waymark6-design.md` §1–§2): **the calendar the plan
...
```bash
make mealplan6                               # db + wheel + uvicorn on :8004
open http://127.0.0.1:8004/                  # the generic human client
```
...
This app runs on **waymark6** (see `docs/waymark6-design.md`). Meal,
...
```bash
uv run waymark6 check mealplan6.main:engine
make conformance6                     # pytest --waymark6 -n auto
uv run pytest --waymark6 plan         # or meal, event, rotation, …
```
