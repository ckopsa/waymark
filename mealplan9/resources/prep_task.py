...
class PrepData(BaseModel):
    ...
    # the deliverable-tracker fact in miniature (design §2, §3): the clock
    # flips it — no write, no poll — and ?overdue=true is one indexed
    # filter instead of every client re-deriving "past due"
    # 8.0: the clock comparison as data (Clock binds as "now"); the flip
    # stays extractable exactly as before — over= is unchanged
    overdue: bool = Derived(over=("due_at", Clock),
                            expr=E.f("now") > E.f("due_at"))
    duration_minutes: int | None = Field(default=None, ge=0)
    # 6.0: the calendar is a kind in this engine (resources/event.py), so
    # the linkage is a real Ref — navigable, dangling-checked — not a
    # string an external system once minted
    calendar_event_id: Ref["event"] | None = RefField(
...
class ScheduleInput(BaseModel):
    # named for what it is: the id of the NEW event just created — not an
    # edit of the stored one, so no prefill and no Edit declaration. A real
    # Ref now that the calendar lives here (6.0): the picker offers events,
    # and a dangling id is refused where a bare string would have slept
    event_id: Ref["event"] = RefField(
        min_length=1,
        description="The calendar event that was created for this task")
