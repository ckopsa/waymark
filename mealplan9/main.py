"""The family meal-planning app: resource definitions in, everything else out.
    ...
    uv run uvicorn mealplan9.main:app --reload
...
from .resources.substitution import Substitution
from .services import Services

def _dsn() -> str:
    """MEALPLAN_DSN from the environment, falling back to a repo-root .env
    (one KEY=VALUE per line) so `uv run uvicorn mealplan9.main:app` works
    ...
    # 2.0 tables differ from v1's (waymark9_* engine tables, timestamptz
    # generated columns) — default to a fresh database rather than colliding
    # with a v1 deployment's schema
    return "postgresql+asyncpg://localhost/mealplan9_dev"
...
from waymark9.server import openapi as _openapi  # noqa: E402

_openapi.install(app, engine.registry)

# The server hands clients their client: built wheels (waymark9 CLI) are
...
if os.path.isdir(CLI_DIR):
    ...
    def cli_index(request: Request) -> PlainTextResponse:
        ...
        lines = ["# waymark9 client — install with one of:", ""]
        for wheel in wheels:
            lines += [f"uv tool install {base}/cli/{wheel}",
                      f"pipx install {base}/cli/{wheel}"]
        lines += ["", f"# then: WAYMARK_BASE={base} waymark9 client index",
