...
as a list), or :class:`Clock` for ``now``. ``within=Tolerance(...)`` is
...
def Derived(
    ...
    **kwargs: Any,
    ...
    for inp in inputs:
        if inp is Clock or isinstance(inp, (str, ChildField)):
            continue
        raise DefinitionError(
...
def Count(edge: Any, *, where: Mapping[str, Any] | None = None,
    ...
    of owned children (matching ``where``), maintained as a field."""
...
def Sum(edge: Any, of: str, *, where: Mapping[str, Any] | None = None,
        **kwargs: Any) -> Any:
    """E4's sum rollup as a library derivation (design §10): Σ of a child
    field across the owned children (matching ``where``)."""
...
async def resolve_inputs(spec: DerivedSpec, r: Any, ctx: Any) -> list[Any]:
    ...
    for inp in spec.over:
        if inp is Clock:
            ...
            args.append([
                child.state if inp.field == "state"
                else child.id if inp.field == "id"
                else getattr(child.data, inp.field)
                for child in rows
            ])
        else:
