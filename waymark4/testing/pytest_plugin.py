"""Pytest plugin: ``pytest --waymark4`` runs the 2.0 conformance suite
...
- a ``waymark4_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm4``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark4") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark4.testing.conformance", "waymark4.testing.walker"):
        return
...
async def wm4(waymark4_engine):
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark4-conformance")
    try:
