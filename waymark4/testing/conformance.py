...
Collected when pytest runs with ``--waymark4``. Parametrized per resource ×
...
async def make(wm4: Any, kind: str, state: str) -> Any:
    try:
        instance = await reg.make_state(kind, state, wm4.engine)
...
async def fetch(wm4: Any, kind: str, id: str, pname: str) -> dict[str, Any]:
    rdef = wm4.registry[kind]
    res = await wm4.client.get(f"{wm4.base}/{rdef.plural}/{id}", headers=H(pname))
    assert res.status_code == 200, res.text
    return res.json()


async def build_input(wm4: Any, kind: str, defn: ActionDef,
    ...
    try:
        return await reg.synthesize_input(wm4.engine, wm4.registry[kind],
                                          defn, instance)
    except reg.SkipState as e:
        pytest.skip(str(e))
...
async def post(wm4: Any, doc: dict[str, Any], defn: ActionDef, pname: str,
    ...
    return await wm4.client.post(href, json=body, headers=headers)


async def storage_version(wm4: Any, kind: str, id: str) -> int:
    async with wm4.storage.session() as s:
        loaded = await wm4.storage.load(s, kind, id)
    return loaded.version


async def transition_count(wm4: Any) -> int:
    from sqlalchemy import func, select

    async with wm4.storage.session() as s:
        return (await s.execute(
            select(func.count()).select_from(wm4.storage.transitions))).scalar_one()


async def principals_with(wm4: Any, kind: str, instance_state: str,
    ...
    out = []
    for pname in sorted(wm4.principals):
        instance = await make(wm4, kind, instance_state)
        doc = await fetch(wm4, kind, instance.id, pname)
...
async def test_representation(wm4, wm3_case):
    ...
    schema_res = await wm4.client.get(f"{wm4.base}/schemas/{kind}")
    assert schema_res.status_code == 200
    jsonschema.validate(doc["data"], schema_res.json())

    assert doc["meta"]["version"] == await storage_version(wm4, kind, instance.id)
...
async def test_affordance_completeness(wm4, wm3_case):
    kind, state, pname = wm3_case
    rdef: ResourceDef = wm4.registry[kind]
    instance = await make(wm4, kind, state)
    doc = await fetch(wm4, kind, instance.id, pname)
    ...
    for name in hidden:
        ...
        body = await build_input(wm4, kind, defn, instance)
        res = await post(wm4, doc, defn, pname, body)
...
async def test_token_prose(wm4, wm3_case):
...
async def test_orientation(wm4, wm3_kind):
    ...
    for state in rdef.machine.states:
        try:
            ...
            continue
        doc = await fetch(wm4, kind, instance.id, sorted(wm4.principals)[0])
...
# ── Transition truth ────────────────────────────────────────────────────
async def test_transition_truth_available(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm4.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm4, kind, state, action, "actions")
    if not cases:
    ...
    for pname, instance, doc in cases:
        ...
        async with wm4.storage.session() as s:
            last = await wm4.storage.last_transition(s, kind, instance.id)
        assert last.action == action
        assert last.from_state == state and last.to_state == defn.to
        assert last.actor_id == wm4.principals[pname].id
        assert last.actor_type == wm4.principals[pname].type

        refetched = await fetch(wm4, kind, instance.id, pname)
        for field in ("state", "data", "meta", "actions"):
            assert refetched[field] == after[field], \
                "response is not the post-transition document"


async def test_transition_truth_unavailable(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm4.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm4, kind, state, action, "unavailable")
    if not cases:
...
async def test_touch_truth(wm4, wm3_action_case):
    ...
    async with wm4.storage.session() as s:
        root = await wm4.storage.last_transition(s, kind, instance.id)
        story = await wm4.storage.transitions_by_correlation(
            s, root.correlation_id)
    actor = wm4.principals[pname].id
    ...
    for t in defn.touches:
        if isinstance(t, Creates):
            created = wm4.registry.get(t.kind)
...
# ── Safety truth ────────────────────────────────────────────────────────
async def test_safety_idempotent_double_invoke(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm4.registry[kind].machine.actions[action]
    if not defn.safety.idempotent:
    ...
    pname, instance, doc = cases[0]
    body = await build_input(wm4, kind, defn, instance)
    first = await post(wm4, doc, defn, pname, body)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200
    v1 = first.json()["meta"]["version"]

    second = await post(wm4, doc, defn, pname, body,
...
async def test_safety_non_idempotent_key_discipline(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm4.registry[kind].machine.actions[action]
    ...
    key = uuid.uuid4().hex
    first = await post(wm4, doc, defn, pname, body, idem_key=key)
    fail_if_needs_example(first, kind, defn)
    assert first.status_code == 200, first.text

    replay = await post(wm4, doc, defn, pname, body, idem_key=key)
    ...
    if defn.input is not None:
        ...
        reuse = await post(wm4, doc, defn, pname, mutated, idem_key=key)
        assert reuse.status_code == 409, \
            f"key reuse with different body must 409, got {reuse.status_code}"


async def test_safety_if_match(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm4.registry[kind].machine.actions[action]
    ...
    pname, instance, doc = cases[0]
    body = await build_input(wm4, kind, defn, instance)
    stale = await post(wm4, doc, defn, pname, body, etag='W/"conformance-stale"')
...
async def test_safety_reversible(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    defn = wm4.registry[kind].machine.actions[action]
...
# ── Input contract ──────────────────────────────────────────────────────
async def test_input_contract(wm4, wm3_action_case):
    kind, state, action = wm3_action_case
    rdef = wm4.registry[kind]
    defn = rdef.machine.actions[action]
    cases = await principals_with(wm4, kind, state, action, "actions")
    ...
    if defn.input is not None:
        junk = await post(wm4, doc, defn, pname, {"__conformance_bogus__": 1})
    ...
    body = await build_input(wm4, kind, defn, instance)
    v_before = await storage_version(wm4, kind, instance.id)
    t_before = await transition_count(wm4)
    dry = await post(wm4, doc, defn, pname, body, dry_run=True)
    ...
    assert await storage_version(wm4, kind, instance.id) == v_before, \
        "dry_run changed the resource version"
    assert await transition_count(wm4) == t_before, \
...
async def test_schema_guard_gap(wm4, wm3_action_case):
    ...
    async with wm4.storage.session() as s:
        ctx = wm4.engine.invoker._ctx(wm4.principals[pname], s, mode="dry_run")
        for g in derivable:
            denies = 0
            for inp in parsed:
...
async def test_prefill_truth(wm4, wm3_action_case):
...
async def test_draft_protection(wm4, wm3_action_case):
    ...
    href = advert["href"]
    empty = await wm4.client.get(href, headers=H(pname))
    ...
    body = await build_input(wm4, kind, defn, instance)
    res = await wm4.client.put(href, json=body, headers=H(pname))
    assert res.status_code == 200, res.text
    saved = res.json()
    ...
    fresh = await wm4.client.get(href, headers=H(pname))
    ...
    for other in sorted(wm4.principals):
        if other == pname:
            continue
        other_get = await wm4.client.get(href, headers=H(other))
    ...
    junk = await wm4.client.put(href, json={"__not_a_field__": 1},
                                headers=H(pname))
    assert junk.status_code == 422, \
        "draft fields outside the action's schema must be rejected"

    res = await post(wm4, refetched, defn, pname, body)
    fail_if_needs_example(res, kind, defn)
    assert res.status_code == 200, res.text
    after_get = await wm4.client.get(href, headers=H(pname))
...
# ── Collection contract ─────────────────────────────────────────────────
async def test_collection_contract(wm4, wm3_kind):
    kind = wm3_kind
    rdef = wm4.registry[kind]
    cls = rdef.cls
    made = []
    for state in cls.__waymark_machine__.states:
        try:
            made.append(await reg.make_state(kind, state, wm4.engine))
        except reg.SkipState:
            continue
    if not made:
        pytest.skip("no states producible")
    pname = sorted(wm4.principals)[0]
    base = f"{wm4.base}/{rdef.plural}"

    async def query(qs: str) -> dict:
        res = await wm4.client.get(f"{base}?{qs}", headers=H(pname))
        assert res.status_code == 200, res.text
        return res.json()
    ...
    while True:
        ...
        if not nxt:
            break
        res = await wm4.client.get(nxt["href"], headers=H(pname))
        assert res.status_code == 200, res.text
        doc = res.json()
...
async def test_bulk_report(wm4, wm3_kind):
    ...
    for defn in bulk_actions:
        ...
        body = dict(await build_input(wm4, kind, defn) or {})
        body["ids"] = ids
        headers = H(sorted(wm4.principals)[0])
        if not defn.safety.idempotent:
            headers["Idempotency-Key"] = uuid.uuid4().hex
        res = await wm4.client.post(f"{wm4.base}/{rdef.plural}/-/{defn.name}",
...
# ── Events ──────────────────────────────────────────────────────────────
async def test_events(wm4, wm3_kind):
    ...
    rdef = wm4.registry[kind]
    machine = rdef.cls.__waymark_machine__
    instance = await make(wm4, kind, machine.initial)
    ...
    sub = wm4.engine.dispatcher.subscribe(resource=(kind, instance.id))
    try:
        body = await build_input(wm4, kind, defn, instance)
        res = await post(wm4, chosen_doc, defn, principal, body)
        fail_if_needs_example(res, kind, defn)
        assert res.status_code == 200, res.text
        after = res.json()
...
async def test_migration_roundtrip(wm4):
    ...
    snapshot = wm4.storage.schema_snapshot()
    stmts = m.diff_statements(wm4.storage, None)

    scratch = "waymark4_conformance_migration"
    async with wm4.storage.engine.begin() as conn:
...
# ── Suite integrity ─────────────────────────────────────────────────────
async def test_every_resource_is_enrolled(wm4):
    missing = [k for k in wm4.registry.kinds() if k not in reg.resources()]
    assert not missing, (
