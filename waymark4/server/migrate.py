...
- ``waymark4 migrate`` snapshots the registry's storage schema
...
- Revisions are plain ``NNNN_*.sql`` files under ``migrations/waymark4/``,
  applied in order and recorded in ``waymark4_schema_migrations`` —
...
MIGRATIONS_TABLE = "waymark4_schema_migrations"
...
def emit(storage: PostgresStorage, directory: Path, *,
    ...
    header = ("-- waymark4 migration: generated from the declared registry.\n"
...
async def apply(dsn: str, directory: Path) -> list[str]:
    ...
    ``waymark4_schema_migrations``. Refuses files with unresolved REVIEW
