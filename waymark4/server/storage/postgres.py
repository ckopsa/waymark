...
- Engine tables are ``waymark4_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark4 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark4_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark4_transitions", self.metadata,
            ...
            Index("ix_waymark4_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark4_idempotency", self.metadata,
        ...
        self.webhook_cursors = Table(
            "waymark4_webhook_cursors", self.metadata,
        ...
        self.cursors = Table(
            "waymark4_cursors", self.metadata,
        ...
        self.job_leases = Table(
            "waymark4_job_leases", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark4 migrate``'s
