...
class Engine:
    def __init__(
        ...
        *,
        ...
        if deploy not in ("auto", "propose"):
            raise ValueError("deploy must be 'auto' or 'propose'")
        # the deploy mode (design 7.0 §1): "auto" keeps the v6 boot revise
        # exactly; "propose" registers an unknown fingerprint as a
        # `proposed` revision and keeps serving the current law when the
        # diff is data-law only (the §1 overlay)
        self.deploy = deploy
    ...
    async def startup(self) -> None:
        ...
        from .definitions import DefinitionLifecycle, revise_definitions

        # the definition-lifecycle seam (design 7.0 §1–§2): the promote/
        # measure handlers and the residency guard consult it through ctx;
        # the invoker's entry point calls its after-commit half
        self.invoker.lifecycle = DefinitionLifecycle(self)
        ...
        if self.invoker.derived.clocked_kinds or has_discovery(self.registry):
            ...
            self._tick_task = asyncio.create_task(
                self._derived_ticks(), name="waymark8-clock")
        ...
        if self._deferred_backfills:
            ...
            self._backfill_task = asyncio.create_task(
                self._drain_backfills(), name="waymark8-backfill")
    ...
    async def _drain_backfills(self) -> None:
        ...
        for kind, declared in self._deferred_backfills:
            try:
                ...
                logging.getLogger("waymark8.derived").exception(
