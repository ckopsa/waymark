...
- ``waymark8 migrate`` snapshots the registry's storage schema
...
- Revisions are plain ``NNNN_*.sql`` files under ``migrations/waymark8/``,
  applied in order and recorded in ``waymark8_schema_migrations`` —
...
MIGRATIONS_TABLE = "waymark8_schema_migrations"
...
def emit(storage: PostgresStorage, directory: Path, *,
    ...
    header = ("-- waymark8 migration: generated from the declared registry.\n"
...
async def apply(dsn: str, directory: Path) -> list[str]:
    ...
    ``waymark8_schema_migrations``. Refuses files with unresolved REVIEW
