...
- Engine tables are ``waymark8_*`` (a v1 and a v2 app can share a database).
...
- :meth:`schema_snapshot` serializes the declared schema — the input to
  ``waymark8 migrate``'s diff (design §8).
"""
...
NOTIFY_CHANNEL = "waymark8_transitions"
...
class PostgresStorage:
    def __init__(self, engine: AsyncEngine | str, registry: Registry):
        ...
        self.transitions = Table(
            "waymark8_transitions", self.metadata,
            ...
            # the waymark8_schema_migrations mechanism: the column is
            # nullable, so the emitted ALTER is a plain ADD COLUMN.
            Column("inputs", JSONB, nullable=True),
            Index("ix_waymark8_transitions_resource", "kind", "resource_id", "id"),
        )
        self.idempotency = Table(
            "waymark8_idempotency", self.metadata,
        ...
        self.webhook_cursors = Table(
            "waymark8_webhook_cursors", self.metadata,
        ...
        self.cursors = Table(
            "waymark8_cursors", self.metadata,
        ...
        self.job_leases = Table(
            "waymark8_job_leases", self.metadata,
    ...
    def schema_snapshot(self) -> dict[str, Any]:
        """The declared schema as data: the input to ``waymark8 migrate``'s
