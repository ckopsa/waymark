"""Pytest plugin: ``pytest --waymark8`` runs the 2.0 conformance suite
...
- a ``waymark8_engine`` async fixture (fresh, isolated storage per test);
...
All fixture names are ``wm8``-prefixed so the v1 and v2 plugins coexist in
one repository.
"""
...
def _selected_kinds(config: pytest.Config) -> list[str]:
    selector = config.getoption("--waymark8") or "*"
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if metafunc.module is None or metafunc.module.__name__ not in (
            "waymark8.testing.conformance", "waymark8.testing.walker"):
        return
...
async def wm8(waymark8_engine):
    ...
    # (waymark8.server.external.refresh_mirror) suppresses pull-through for
    ...
    client = AsyncClient(transport=transport, base_url="http://waymark8-conformance")
    try:
